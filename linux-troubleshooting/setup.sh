...
# ── 2. 先讓 nginx 正常跑一次，確保基礎環境沒問題 ──
systemctl enable nginx >/dev/null 2>&1
systemctl start nginx  >/dev/null 2>&1
sleep 2

# ── 3. Stage 4 用：報表目錄（父目錄故意不給 others 執行權）──
...
# ── 5. Stage 5 用：製造一個肥大的 log 檔 ─────────────
mkdir -p /var/log/nginx
head -c 200M /dev/zero > /var/log/nginx/error.log 2>/dev/null || \
  dd if=/dev/zero of=/var/log/nginx/error.log bs=1M count=200 >/dev/null 2>&1
echo "2026/07/10 01:30:02 [error] 1234#0: *1 open() \"/var/www/html/reports/report.txt\" failed (13: Permission denied)" >> /var/log/nginx/error.log

# ── 6. 埋下災難：設定檔漏掉一個分號 ──────────────────
CONF=/etc/nginx/sites-enabled/default
sed -i '0,/listen 80 default_server;/s//listen 80 default_server/' "$CONF"

# ── 7. 讓服務在學生進來時已經是 failed ─────────────
